...
| Etária | data de nascimento (ou faixa "maior/menor de 18") | Aluno |
...
| Data de nascimento | determinar se o aluno é menor de 18 (aciona o art. 14) | **coletar só a faixa "maior/menor de 18"** — recomendado adotar esta opção no MVP; a data completa não é necessária à finalidade |
...
**Ação recomendada:** trocar "data de nascimento" por "faixa etária (maior/menor de 18)" no fluxo de cadastro, salvo se a data for necessária para outra finalidade identificada.
...
O MVP confia na **declaração do professor** de que obteve o consentimento do responsável — não há verificação documental forte (upload do termo, confirmação por e-mail ao responsável). Decisão consciente para o MVP (registrada em `OPEN-QUESTIONS.md`), com **plano de reforço pós-lançamento**:
...
| R3 | Vazamento por credencial de professor comprometida | Média | Alto | senha e sessão geridas pelo Firebase Auth; App Check nas funções `callable`; registros de acesso; **[pendente]** avaliar MFA opcional para professor | Médio |
| R4 | Dados de aluno menor tratados sem consentimento válido do responsável (declaração falsa ou equivocada do professor) | Média | Alto | fluxo RF-021 (sem autocadastro de menor); termo modelo fornecido; registro `GUARDIAN_CONSENT`; Política de Privacidade explícita; **plano de reforço da seção 4** | Médio |
| R5 | Coleta ou retenção excessiva de dados de menores | Média | Médio | minimização (seção 3.2); recomendação de coletar só faixa etária; `purgeExpiredData` aplica a retenção | Baixo |
...
| R9 | Incidente de segurança sem contenção/notificação adequada | Baixa | Alto | Cloud Logging + Error Reporting; plano de resposta a incidentes com notificação à ANPD e aos titulares (`plano-resposta-incidentes.md`, Fase 6) | Médio |
...
- **R3 (credencial de professor):** decidir sobre MFA opcional.
- **R4 (consentimento parental):** aceitar o risco médio no MVP com o plano de reforço, ou antecipar a confirmação por e-mail ao responsável.
- **R9 (resposta a incidente):** o plano precisa existir e ser testado antes do go-live.
...
- Plano de resposta a incidentes **[pendente — Fase 6]**.
- Política de retenção formal **[pendente]**.
...
### Itens pendentes para a versão final

1. Nome / CPF ou CNPJ da controladora.
2. Confirmar prazos de retenção (`politica-de-retencao.md`).
3. Trocar "data de nascimento" por "faixa etária" no cadastro, ou justificar a coleta da data.
4. Decidir sobre MFA para professor (R3).
5. Escrever e testar o plano de resposta a incidentes (R9).
6. Definir o reforço do consentimento parental e seu prazo (R4).
7. Revisão jurídica.
