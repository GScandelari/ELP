...
export const exportUserData = onCall(async (request) => {
  ...
  if (role === "teacher") {
    ...
    data.classes = classesSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
    data.activities = activitiesSnap.docs.map((d) => ({
      id: d.id,
      ...d.data(),
    }));
  } else {
    const [enrollmentsSnap, attemptsSnap, resultsSnap] = await Promise.all([
      db.collectionGroup("enrollments").where("studentId", "==", uid).get(),
      db.collection("attempts").where("studentId", "==", uid).get(),
      db.collection("attemptResults").where("studentId", "==", uid).get(),
    ]);
    ...
    for (const doc of attemptsSnap.docs) {
      ...
      attempts.push({
        id: doc.id,
        ...doc.data(),
        answers: answersSnap.docs.map((a) => ({ itemId: a.id, ...a.data() })),
      });
    }
  ...
  const consentsSnap = await db.collection(`consents/${uid}/records`).get();
  data.consents = consentsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
